...
export const getUser = asyncHandler(async (req, res, next) => {
  ...
  const following_categories = await Comment.find({
    user_id: user._id,
  }).distinct("category");
...
export const updateCurrentUser = asyncHandler(async (req, res, next) => {
  const user = req.user;
  const { full_name, user_name, email, profile_image_url, bio, location } =
    req.body;
...
export const changePasswordCurrentUser = asyncHandler(
  async (req, res, next) => {
    const user = req.user;
