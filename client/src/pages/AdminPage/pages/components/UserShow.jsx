...
import { Badge, Breadcrumb, Container, Spinner, Button, } from "react-bootstrap";
...
const UserShow = () => {
  ...
  const {data: comments, isLoading: isCommentLoading} = useQuery(getUserCommentsQuery(id, paramsPost.page, paramsPost.limit, paramsPost.order), {
    keepPreviousData: true,
  });
    ...
    <Container className="dashboard-container">
      ...
      <div className="d-flex mb-3 justify-content-end gap-3">
        <Button
          ...
          onClick={() => navigate("/admin/dashboard/users/:id/edit".replace(":id", id))}
        >
          Edit
        </Button>
          ...
          <div className="p-2">
            ...
            <div className="border p-3 mb-3">
              {commentsData?.map((comment, index) => (
                ...
                >
                  <Link to={`/admin/dashboard/comments/${comment._id}`} dangerouslySetInnerHTML={{__html: comment.content}}>
                  </Link>
                </div>
              ))}
            </div>
            ...
            <div className="border p-3 mb-3">
              {data?.liked_post.map((following, index) => (
                <div key={following._id} 
                className="border-bottom"
                >
                  <span>{index + 1}. {" "}</span>
            ...
            <div className="border p-3 mb-3">
              {data?.reading_list.map((following, index) => (
                <div key={following._id} 
                className="border-bottom"
                >
                  <span>{index + 1}. {" "}</span>
            ...
            <div className="border p-3 mb-3">
              {data?.blocked_users.map((following, index) => (
                <div key={following._id} 
                className="border-bottom"
                >
                  <span>{index + 1}. {" "}</span>
            ...
            <Badge className="mb-3" pill>
              {data?.is_comment_blocked ? "Yes" : "No"}
            </Badge>
            ...
            <Badge className="mb-3" pill>
              {data?.is_create_post_blocked ? "Yes" : "No"}
            </Badge>
            ...
            <h6>Updated At</h6>
            <p>{new Date(data?.updatedAt).toUTCString()}</p>
          </div>
        </Container>
      )}
    </Container>
